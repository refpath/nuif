...
use crate::syntax::{escape_jsx_attribute, escape_jsx_text, number};
use crate::{AdapterError, AdapterReport, ExportedSource, PROFILE_NAME};

/// Exports a document in the bounded static intrinsic-JSX profile.
///
/// # Errors
///
/// Returns typed fidelity when the document exceeds the profile and rejects
/// any generated source that does not self-import to the same document.
...
fn render(document: &Document) -> String {
    ...
    source.push_str("export default function NuifDocument() {\n  return (\n");
    render_entity(
        &mut source,
        document,
        document.roots[0],
    ...
    match entity.kind {
        EntityKind::Container => {
            ...
            emit(
                ...
                format_args!(
                    ...
                    entity.id,
                    escape_jsx_attribute(entity.name.as_deref().expect("profile requires name")),
                ),
            );
            ...
            emit(
                source,
                format_args!(
                    " style={{{{ width: {}, height: {}, boxSizing: \"border-box\", display: \"flex\", flexDirection: \"{}\", gap: {}, paddingTop: {}, paddingRight: {}, paddingBottom: {}, paddingLeft: {}, alignItems: \"{}\" }}}}>\n",
                    number(width),
                    number(height),
        ...
        EntityKind::Text => {
            ...
            emit(
                source,
                format_args!(
                    "{indent}<span data-nuif-id=\"{}\" data-nuif-kind=\"text\" data-nuif-name=\"{}\" data-nuif-font-sha256=\"{}\" style={{{{ width: \"100%\", fontFamily: {}, fontSize: {}, lineHeight: {} }}}}>{}</span>\n",
                    entity.id,
                    escape_jsx_attribute(entity.name.as_deref().expect("profile requires name")),
                    text.font_sha256,
                    serde_json::to_string(&text.font).expect("font string serializes"),
                    number(text.size),
                    serde_json::to_string(&format!("{}px", number(text.line_height)))
                        .expect("line height string serializes"),
                    escape_jsx_text(&text.content),
                ),
            );
        }
