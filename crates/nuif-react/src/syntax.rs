...
#[must_use]
pub fn escape_jsx_text(value: &str) -> String {
    value
        .replace('&', "&amp;")
        .replace('<', "&lt;")
        .replace('>', "&gt;")
...
#[must_use]
pub fn escape_jsx_attribute(value: &str) -> String {
    escape_jsx_text(value).replace('"', "&quot;")
}

pub fn unescape_jsx(value: &str, pointer: &str) -> Result<String, AdapterError> {
    ...
    while let Some(index) = remainder.find('&') {
        ...
        .ok_or_else(|| AdapterError::InvalidValue {
            ...
            reason: "profile accepts only its canonical JSX entity escapes".to_owned(),
        })?;
        output.push(decoded);
