...
/// Applies mapped token changes as byte-local edits to retained DTCG JSON.
///
/// # Errors
///
/// Returns typed stale-span, unsupported, structural, parse or synchronization
/// errors without returning partial source.
pub fn synchronize(
    ...
    if structure_signature(&retained.document) != structure_signature(edited) {
        return Err(unmapped(
            edited,
            "document identity or token inventory changed",
        ));
    }
...
fn structure_signature(document: &Document) -> (EntityId, Vec<EntityId>) {
    (document.id, document.tokens.keys().copied().collect())
}
