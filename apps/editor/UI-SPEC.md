...
- Import: the File menu selects NUIF, SVG, HTML/CSS, DTCG, Penpot or static React JSX. External input is size-bounded before parsing; text profiles require UTF-8 while the Penpot profile validates bounded ZIP members in memory. A confirmation dialog shows fidelity-class and correspondence counts before the imported document replaces the session as an unsaved document. Merge-into-document import is not implemented.
- Export: the File menu selects PNG, SVG, HTML/CSS, DTCG, Penpot or static React JSX. Each external adapter export writes the artefact and a sibling `.report.json`; a profile mismatch fails before the destination chooser and writes nothing.
