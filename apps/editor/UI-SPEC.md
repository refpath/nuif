...
- Import: the File menu selects NUIF, SVG, HTML/CSS or DTCG. External input is size-bounded before UTF-8 parsing, and a confirmation dialog shows fidelity-class and correspondence counts before the imported document replaces the session as an unsaved document. Merge-into-document import is not implemented.
- Export: the File menu selects PNG, SVG, HTML/CSS or DTCG. Each external adapter export writes the artefact and a sibling `.report.json`; a profile mismatch fails before the destination chooser and writes nothing.
