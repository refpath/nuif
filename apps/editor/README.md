...
The executable profile-zero shell provides document open/save, page creation, layer and component browsing, identity-backed canvas selection, frame/rectangle/ellipse/path/text insertion, subtree duplication and deletion, undo/redo, evaluation-width presets, zoom, panel visibility, a command palette and PNG snapshots. Its inspector authors names, positions, sizing intents, stack/flex layout, gaps, four-edge padding, alignment, solid fills and pinned-font text. Multi-field Apply is one atomic transaction.
