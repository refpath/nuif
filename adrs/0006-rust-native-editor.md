...
2. Masonry is pinned by full SHA to the reviewed `refpath/xilem` fork at `1b96eb8`; its parent `eabfe0a` carries the wgpu 29 migration, and it descends from the evaluated upstream main revision `b81d8d7` (2026-08-28). The 0.4.0 release of 2025-10-29 lacks the `Canvas` widget, paints into a Vello 0.6 `Scene`, and screenshots only through wgpu. The pinned line provides `Widget::paint` over an `imaging::Painter`, `Canvas::update_scene` recording an `imaging::record::Scene`, and `masonry_testing` rasterization through `imaging_vello_cpu` without a GPU.
