...
After `cargo xtask browser-install`, `cargo xtask gate-c` lowers this fixture independently to Taffy and DOM/CSS at 360 × 640, 768 × 768 and 1,440 × 900. The strict 2026-08-29 report records exact agreement for every box across all three engines. `cargo xtask gate-g` separately compares canonical text, opaque preservation, layout, decoded RGBA and fidelity with the standard-library-only Python implementation. The full-v0 HTML/CSS retentive source-synchronization segment remains the experiment's incomplete falsifier; browser layout lowering and the independent NUIF reader/evaluator do not satisfy that source-path requirement.
