...
8. Keep the variable-font metadata/normalization and isolated HarfBuzz shaping
   ...
   evidence plus representative packed `gvar` negatives and the independently
   authored Noto Sans/Recursive OFL corpus, then strengthen generated packed
   encodings before typed package/runtime coordinate delivery.
...
13. Run the bounded Figma and Canva profiles in named live hosts and the
    Affinity SVG bridge as a retained user-mediated foreign-runtime trial. The
    smallest credential-free Canva Apps SDK shell is now compiled, measured and
    packaged with its restrictive SDK license after the pure mapper remains
    green; its one-sync behavior is still mock evidence. Retain host reports and
    never infer live behavior, marketplace approval or native NUIF support from
    static code or API documentation.
