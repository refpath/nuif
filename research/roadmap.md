...
4. Keep the tested Masonry shell attached only through the editor driver boundary; extend the implemented one-transaction freeform move and south-east fixed-resize gestures to the remaining handles, aspect-ratio behavior and managed-layout reorder without moving semantic rules into shell code.
